...
def extract_untranslated_trans_units(input_file):
    ...
    for line in lines:
        if "<trans-unit" in line:
            ...
            if "</trans-unit>" in line:
                inside_trans_unit = False
                # Check if the trans-unit contains an untranslated target or an empty target
                if any("[NAB: NOT TRANSLATED]" in l.upper() for l in current_trans_unit) or \
                   any("<target></target>" in l for l in current_trans_unit):
    ...
    print(f"Extracted {len(untranslated_units)} untranslated or empty-target trans-units to {output_file}")
